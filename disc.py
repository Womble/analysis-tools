...
def genetic_double_gaussian_fit(im) :
     "fit double genetic profiles to the each of the spectra in imcube im"
     shape=im.shape
...
def disc_analysis (path, threshold=0.1):
...
the results to a new fits file called Xfitted.fits of shape [4,x,y] where the 4 planes of the output are mean of the first and second gaussians"""
    ...
    ans=np.ones([4]+res.shape[1:])*-1
    shape=res.shape
    for i in xrange(shape[1]):
        for j in xrange(shape[2]):
             ans[2:3,i,j]=res[2:6:3,i,j]
             if abs(res[0,i,j]) > abs(res[3,i,j])  and abs(res[0,i,j])/abs(res[6,i,j]) > threshold:
                  ans[0,i,j]=res[2,i,j]
             if abs(res[0,i,j]) <= abs(res[3,i,j]) and abs(res[3,i,j])/abs(res[6,i,j]) > threshold:
                  ans[1,i,j]=res[5,i,j]
    return ans
